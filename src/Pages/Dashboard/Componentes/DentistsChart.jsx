...
const METRICS = {
  count: {
    label: "Atendimentos",
    endpoint: "dentist-appointments-count",
    valueKey: "appointments_count",
    format: (v) => `${v}`,
  },
  billing: {
    label: "Valor gerado",
    endpoint: "dentist-billing-summary",
  ...
  return (
    <div className="chart-tooltip">
      ...
      <div className="chart-tooltip-row">
        <span className="chart-tooltip-dot" style={{ background: "#22C55E" }} />
...
export default function DentistsChart({ startDate, endDate }) {
  const { token } = useAuth();
  ...
  return (
    <div className="chart-card chart-card-half">
      <div className="chart-card-header">
        <h3 className="chart-card-title">Desempenho dos dentistas</h3>
        <select
        ...
        {loading ? (
          ...
          <>
            ...
            {hasMore && (
              ...
              >
                {showAll ? "Ver menos" : `Ver todos os dentistas (${data.length})`}
              </button>
            )}
          </>
        )}
